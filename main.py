...
from PyQt5.QtGui import QWindow, QFont, QColor, QPainter, QBrush
...
# ==========================================
# ANIMASI LAYAR LOADING rainX (SPLASH)
# ==========================================
class RainXLoadingScreen(QWidget):
    ...
    def init_ui(self):
        ...
        font_rain = QFont("Segoe UI", 48, QFont.Light)
        self.lbl_rain.setFont(font_rain)
        self.lbl_rain.setStyleSheet("color: #A0A0A5;")
        
        self.lbl_x = QLabel("X")
        font_x = QFont("Segoe UI", 48, QFont.ExtraBold)
        self.lbl_x.setFont(font_x)
        self.lbl_x.setStyleSheet("color: #00E5FF;")
        ...
        font_loading = QFont("Segoe UI", 9, QFont.DemiBold)
        font_loading.setLetterSpacing(QFont.AbsoluteSpacing, 4)
...
class PasscodeUnlockDialog(QDialog):
    ...
    def execute_unlock(self):
        ...
        try:
            if method_idx == 0:
                subprocess.run(["adb", "-s", self.device_id, "shell", "input", "keyevent", "KEYCODE_WAKE"], startupinfo=startupinfo, check=True)
                time.sleep(0.3)
                subprocess.run(["adb", "-s", self.device_id, "shell", "input", "swipe", "300", "1000", "300", "300"], startupinfo=startupinfo, check=True)
                if pin_text:
                    time.sleep(0.5)
                    subprocess.run(["adb", "-s", self.device_id, "shell", "input", "text", pin_text], startupinfo=startupinfo, check=True)
                    subprocess.run(["adb", "-s", self.device_id, "shell", "input", "keyevent", "66"], startupinfo=startupinfo, check=True)
                QMessageBox.information(self, "Sukses", "Perintah KeyEvent Buka Kunci berhasil dikirim.")
                
            elif method_idx == 1:
                subprocess.run(["adb", "-s", self.device_id, "shell", "wm", "dismiss-keyguard"], startupinfo=startupinfo, check=True)
                ...
                if reply == QMessageBox.Yes:
                    subprocess.run(["adb", "-s", self.device_id, "reboot", "recovery"], startupinfo=startupinfo, check=True)
...
class AppManagerDialog(QDialog):
    ...
    def load_packages(self):
        ...
        try:
            out_user = subprocess.check_output(["adb", "-s", self.device_id, "shell", "pm", "list", "packages", "-3"], creationflags=creation_flags).decode("utf-8")
            ...
            out_sys = subprocess.check_output(["adb", "-s", self.device_id, "shell", "pm", "list", "packages", "-s"], creationflags=creation_flags).decode("utf-8")
    ...
    def uninstall_selected(self):
        ...
        if reply == QMessageBox.Yes:
            ...
            try:
                ...
                output = subprocess.check_output(cmd, creationflags=creation_flags).decode("utf-8")
...
class GlassDeviceWall(QWidget):
    ...
    def init_ui(self):
        ...
        self.btn_screen_toggle = QPushButton("📱 Always On GUI + Device Sleep: OFF", self)
    ...
    def toggle_screen_setting(self):
        ...
        if self.auto_off_screen:
            self.btn_screen_toggle.setText("📱 Always On GUI + Device Sleep: ON")
            ...
            self.btn_screen_toggle.setText("📱 Always On GUI + Device Sleep: OFF")
            if self.is_dark_mode:
                self.btn_screen_toggle.setStyleSheet("QPushButton { background: #3A3A3C; border: 1px solid #48484A; color: #E5E5EA; padding: 5px 12px; border-radius: 5px; font-weight: 600; }")
            else:
                self.btn_screen_toggle.setStyleSheet("QPushButton { background: #E5E5EA; border: 1px solid #C7C7CC; color: #1C1C1E; padding: 5px 12px; border-radius: 5px; font-weight: 600; }")
    ...
    def update_page_ui(self):
        ...
        for index, dev_id in enumerate(page_devices):
            ...
            cmd = ["scrcpy", "-s", dev_id, "--window-title", f"scrcpy_{dev_id}"]
            if self.config_res != "Auto":
                cmd.extend(["-m", self.config_res])
            if self.audio_muted:
                cmd.append("--no-audio")
            
            # IMPLEMENTASI KONDISI PERMINTAAN:
            # --turn-screen-off membuat layar fisik sleep segera setelah terkoneksi.
            # --stay-awake menjaga transmisi data/video tetap 'Always On' di GUI PC.
            # Saat diskonek (proses di-terminate / kabel dicabut), scrcpy bawaan akan otomatis mengembalikan layar fisik HP menyala normal.
            if self.auto_off_screen:
            ...
            try:
                ...
                print(f"Gagal menjalankan scrcpy untuk {dev_id}: {e}")
    ...
    def mass_sideload_apk(self):
        ...
        if not self.device_ids:
            QMessageBox.warning(self, "Gagal", "Tidak ada perangkat terdeteksi untuk instalasi massal.")
            return
        ...
        for dev_id in self.device_ids:
            try:
                res = subprocess.run(["adb", "-s", dev_id, "install", "-r", file_path], capture_output=True, text=True, startupinfo=startupinfo)
        ...
        QMessageBox.information(self, "Hasil Batch Sideload", f"Proses Selesai!\n\nSukses Terinstall: {success_count} HP\nGagal: {fail_count} HP")
...
def get_connected_devices():
    try:
        ...
        out = subprocess.check_output(["adb", "devices"], creationflags=creation_flags).decode("utf-8")
        devices = []
...
if __name__ == '__main__':
    ...
    def launch_main_app():
        ...
        main_win = GlassDeviceWall(devices)
        main_win.show()

    splash = RainXLoadingScreen(launch_main_app)
    splash.show()
