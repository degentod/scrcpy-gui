...
class PasscodeUnlockDialog(QDialog):
    """ Dialog Menu Presisi untuk Eksekusi Unlock Passcode """
    def __init__(self, device_id, parent=None):
        ...
        # Input Kunci Tambahan Jika Diperlukan PIN/Pola
    ...
    def execute_unlock(self):
        ...
        try:
            if method_idx == 0:
                # Membangunkan layar dan mengirim perintah usap ke atas
                ...
                if pin_text:
                    ...
                    subprocess.run(["adb", "-s", self.device_id, "shell", "input", "keyevent", "66"], check=True) # Enter
                QMessageBox.information(self, "Sukses", "Perintah KeyEvent Buka Kunci berhasil dikirim.")
                
            elif method_idx == 1:
                # Mengirimkan instruksi pembebasan antarmuka kunci utama
                ...
                # Upaya penghapusan file database sandi sistem lokal (Wajib Root)
                ...
                # Memaksa perangkat masuk langsung ke menu wipe hard reset
            ...
            QMessageBox.critical(self, "Kesalahan Batas Waktu", f"Gagal mengeksekusi operasi bypass:\n{e}")
...
class DeviceCard(QFrame):
    def __init__(self, device_id, main_wall, parent=None):
        ...
        # Sub-grid layout presisi agar scrcpy menempel di sudut-sudut border ungu tanpa teks ID
        ...
        # Dimensi murni murni aspek rasio 16:9 disesuaikan untuk monitor target resolusi 1680p
...
class GlassDeviceWall(QWidget):
    def __init__(self, device_ids):
        super().__init__()
        # Batasan maksimal hingga 20 unit perangkat secara persisten
        self.device_ids = device_ids[:20]
        self.processes = []
        self.cards = []
        
        # Konfigurasi Paginasi Kontrol (4 Device per halaman, Grid 2x2)
        self.current_page = 0
    ...
    def init_ui(self):
        ...
        # --- TOP CONTROL BAR (Gaya Bersih iTunes / 3uTools) ---
        ...
        # Informasi Dropdown Seluruh Perangkat Terkoneksi (Persisten di Bar Atas)
        ...
        # 👑 FITUR BARU: MENU BUTTON UNLOCK PASSCODE (Persisten di Atas Bar)
        if self.device_ids:
        ...
        self.lbl_page_indicator.setStyleSheet("font-weight: bold; color: #8E44AD; min-width: 60px; qproperty-alignment: AlignmentFlag.AlignCenter;")
        ...
        # Area Tampilan Grid Terbuka Bersih 100% untuk Layar Mirroring
    ...
    def open_passcode_unlocker(self):
        """ Membuka Dialog Unlock Passcode untuk perangkat yang sedang dipilih di dropdown """
        target_device = self.combo_all_connected.currentText()
        if target_device and target_device != "No Device Connected":
